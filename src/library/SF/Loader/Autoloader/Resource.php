...
* this will be replaced by Zend_Loader_Autoloader_Resource when it is 
* finished.
*
...
* @version $Id: $
*/
class SF_Loader_Autoloader_Resource implements SF_Loader_Autoloader_Interface
{
    protected $_basePath;
    ...
    public function __construct($options)
    {
        ...
        if (!is_array($options)) {
            throw new SF_Loader_Exception('Options must be passed to resource loader constructor');
        }
        ...
        if ((null === $this->getPrefix())
            ...
            throw new SF_Loader_Exception('Resource loader requires both a prefix and a base path for initialization');
        }
 
        $this->initDefaultResourceTypes();
        SF_Loader_Autoloader::unshiftAutoloader($this);
    }
    ...
    {
        if ('get' == substr(strtolower($method), 0, 3)) {
            $type = strtolower(substr($method, 3));
            if (!$this->hasResourceType($type)) {
                throw new SF_Loader_Exception("Invalid resource type $type; cannot load resource");
            }
            if (empty($args)) {
                throw new SF_Loader_Exception("Cannot load resources; no resource specified");
            }
            $resource = array_shift($args);
            return $this->load($resource, $type);
        }
        throw new SF_Loader_Exception("Method '$method' is not supported");
    }
    ...
    public function addResourceType($type, $path, $prefix = null)
    {
        ...
        if (!isset($this->_resourceTypes[$type])) {
            if (null === $prefix) {
                throw new SF_Loader_Exception('Initial definition of a resource type must include a prefix');
            }
        ...
        if (!is_string($path)) {
            throw new SF_Loader_Exception('Invalid path specification provided; must be string');
        }
    ...
    public function addResourceTypes(array $types)
    {
        foreach ($types as $type => $spec) {
            if (!is_array($spec)) {
                throw new SF_Loader_Exception('addResourceTypes() expects an array of arrays');
            }
            if (!isset($spec['path'])) {
                throw new SF_Loader_Exception('addResourceTypes() expects each array to include a paths element');
            }
            $paths = $spec['path'];
            $prefix = null;
    ...
    public function load($resource, $type = null)
    {
        if (null === $type) {
            ...
            if (empty($type)) {
                throw new SF_Loader_Exception('No resource type specified');
            }
        }
        if (!$this->hasResourceType($type)) {
            throw new SF_Loader_Exception('Invalid resource type specified');
        }
        $prefix = $this->_resourceTypes[$type]['prefix'];
